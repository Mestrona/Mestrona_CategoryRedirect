 ...
 */
namespace Mestrona\CategoryRedirect\Block\Html;
...
class Topmenu extends \Magento\Theme\Block\Html\Topmenu
{
    ...
    protected function _getHtml(
        ...
        foreach ($children as $child) {
            ...
            $html .= '<a href="' . $child->getUrl() . ' " ' . ($this->isExternal($child) === true ? ' target="_blank" ' : '') . $outermostClassCode . '><span>'
    ...
    /**
     * Check if is external or internal by looking at scheme and host
     *
     * @param $child
     * @return bool
     */
    public function isExternal($child)
    {
        $url = parse_url($child->getUrl());
        $domainPattern = '/^(https?:\/\/)?(www\.)?(' . preg_quote($url['host']) . ')(\/)?/i';

        return (!empty($url['scheme']) && !preg_match($domainPattern, $this->getBaseUrl())) ? true : false;
    }
}
